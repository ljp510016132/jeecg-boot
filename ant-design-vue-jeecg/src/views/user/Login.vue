<template>
  <div class="main">
    <a-form :form="form" class="user-layout-login" ref="formLogin" id="formLogin">
     <!-- 登录平台：
        <a-select defaultValue="0000" style="width: 292px" @change="platformChange">
          <a-select-option value="0000">权限管理系统</a-select-option>
          <a-select-option value="0001">基础线网系统</a-select-option>
          <a-select-option value="0002">公交调度系统</a-select-option>
        </a-select> -->
      ...
      <a-form>
        ...
        >
          ...
          >
            ...
            <a-select-option v-for="d in orgList" :key="d.id" :value="d.orgCode">{{ d.orgName }}</a-select-option>
          </a-select>
        </a-form-item>
      </a-form>
    </a-modal>
    ...
    >
      ...
      <a-form>
        ...
        >
          ...
          >
            <a-icon slot="suffixIcon" type="gold" />
            <a-select-option v-for="d in platformList" :key="d.id" :value="d.platformCode">{{ d.platformName }}</a-select-option>
          </a-select>
        </a-form-item>
      </a-form>
    </a-modal>
...
export default {
  ...
  methods: {
          ...
          (err, values) => {
            if (!err) {
              ...
              that
                ...
                //   this.platformOk(res)
                // })
                //将系统选择放在登陆后面，根据用户拥有的系统进行智能推荐
                .then(res => {
                  this.platformConfirm(res)
                })
                // .then(res => {
                //   this.orgConfirm(res)
                // })
    ...
    //需要登陆的子系统选择，如果每个子系统独立，可以直接在这里写死需要登陆的子系统
    platformConfirm(res) {
      if (res.success) {
        ...
        if(multi_platform==0){
          ...
          this.$notification.warn({
            message: '提示',
            description: `您尚未有任何系统权限,请确认账号信息`,
            duration:3
          });
        }else if(multi_platform==2){
    ...
    platformOk(){
      ...
      putAction('/sys/selectPlatform', obj).then(res => {
        if (res.success) {
          ...
          console.log("---当前登录平台---userInfo-------",store.getters.userInfo.platformCode);
          this.platformClear()
          this.loginSuccess()
        } else {
          this.requestFailed(res)
